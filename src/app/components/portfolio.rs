...
pub fn Portfolio(
    ...
    move || {
        portfolios
            ...
            .enumerate()
            .map(|(index, portfolio)| {
                // Define a closure to generate edit_menu
                let edit_menu = move || {
                    if is_edit {
                        Either::Left(
                            view! {
                                ...
                                <button
                                    ...
                                    on:click=move |_| {
                                        if let Some(ref callback) = on_edit {
                                            callback.run(index); // Proper callback emission
                                        }
                                    }
                                >
                ...
                let aLink = if portfolio.portfolio_link.is_empty() {
                    ...
                    Some(
                        view! {
                        <div style="margin-top:2px; align-items: end; color:blue;">
                ...
                view! {
                    <div class="portfolioContainer">
                        <div class="portfolioRow">
                            <div class="portfolioColumn">
                                <div class="portfolioHeader">
                                    ...
                                    <div class="experienceRowFirstItemText">
                                        {if is_mobile.get() { edit_menu() } else { Either::Right(()) }}
                            ...
                            <div>
                                {if is_mobile.get() { edit_menu() } else { Either::Right(()) }}
                                <ImageSlider images=portfolio.screenshots_url.clone()/>
                            </div>
                        </div>
                        <div class="stackRow">
